...
const bestDepIso = f =>
  f?.sobt || f?.eobt || f?.aobt || f?.pobt || f?.ctot || f?.etot || f?.atot || "";
const bestArrIso = f =>
  f?.sibt || f?.eibt || f?.aibt || f?.aldt || f?.eldt || f?.afat || f?.efat || "";
  ...
  LIR_RYANAIR:{
    ...
    fill:({vol1}, extra=null)=>{
      ...
      return {
        ...
        "B737": x.B737, "B738": x.B738, "B38M": x.B38M,
        "SI":                      extra?.si || "",
        "MAX 5":                   max5 ? "MAX 5" : "",
...
async function mergeAndPrint(jobs){
  ...
  // ViewerPreferences duplex — dans un try/catch car PDFName peut ne pas être exposé
  try{
    ...
    if(PDFName){
      merged.catalog.set(
        ...
        merged.context.obj({
          Duplex:       PDFName.of(allDuplex ? "DuplexFlipLongEdge" : "Simplex"),
...
function closeMenageModal(keepPending=false){
...
function openBBCGModal(pending){
  ...
  setTimeout(() => $("bbcgGate")?.focus(), 0);
}
function closeBBCGModal(keepPending=false){
...
function openSIModal(pending){
  ...
  const pPorte = $("siModalPoussettesPorte");
  const pCBS   = $("siModalPoussettesCBS");
...
function closeSIModal(keepPending=false){
...
function closeRZAModal(keepPending=false){
  const hEl = $("rzaHelp");
  if(hEl) hEl.style.display = "none";
...
async function fetchAK(flow, from, to){
  ...
  if(res.type==="opaqueredirect" || res.status===0 || (res.status>=300&&res.status<400) || res.redirected){
...
function buildFlightLabel(f, flow){
  ...
  const dest   = upper(isArr ? (f?.adepIata||f?.adepIcao||"") : (f?.adesIata||f?.adesIcao||""));
  const reg    = upper(f?.reg || "") || "-----";
  const stand  = (f?.pkg||"").toString().replace(/^P/i,"").trim();
  const p      = stand ? `P${stand}` : "";
  const lid    = String(f?.linkedId || "").trim();

  let departed = false, arrived = false;
  if(isArr){
    arrived  = Number.isFinite(Date.parse(f?.aibt || ""));
    const dep = lid ? (_akDepById.get(lid) || null) : null;
    departed  = Number.isFinite(Date.parse(dep?.atot || ""));
  }else{
    departed  = Number.isFinite(Date.parse(f?.atot || ""));
    const arr = lid ? _akArrAll.find(a => String(a?.id||"") === lid) : null;
    arrived   = Number.isFinite(Date.parse(arr?.aibt || ""));
  }

  const status = departed ? "DÉCOLLÉ" : arrived ? "ARRIVÉ" : "";
  const icon   = isArr ? "🛬" : "🛫";
...
function sameDayDepArr(dep, arr){
  const depDay = ymdUTC(bestDepIso(dep));
  const arrDay = ymdUTC(bestArrIso(arr));
...
function findLinkedArrForDepSameDay(dep, arrAll){
  ...
  return arr && sameDayDepArr(dep, arr) ? arr : null;
}
...
function akAcType(f){
  return upper(f?.acTypeIcao || f?.acTypeIata || f?.aircraftDetails?.acTypeName || "");
}
...
function updateBadgesFromFlights(n, depFlight, arrFlight){
  ...
  if(depFlight && $(`dep_badges_${n}`))
    renderDelayBadge($(`dep_badges_${n}`), delayMinFrom(depFlight?.sobt||"", actualDepIso(depFlight)));
  if(arrFlight && $(`arr_badges_${n}`))
    renderDelayBadge($(`arr_badges_${n}`), delayMinFrom(arrFlight?.sibt||"", actualArrIso(arrFlight)));
}
...
async function loadAKAll(){
  ...
  try{
    ...
    _akDepToday = depAll.filter(inTodayDep).sort((a,b)=>(Date.parse(a?.sobt||"")||1e18)-(Date.parse(b?.sobt||"")||1e18));
    _akArrToday = arrAll.filter(inTodayArr).sort((a,b)=>(Date.parse(a?.sibt||"")||1e18)-(Date.parse(b?.sibt||"")||1e18));
    _akArrAll   = arrAll;
    _akDepAll   = depAll;
